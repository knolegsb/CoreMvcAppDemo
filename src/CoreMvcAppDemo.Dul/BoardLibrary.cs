...
namespace CoreMvcAppDemo.Dul
{
    public class BoardLibrary
    {
            ...
            else
            {
                for (int i = 0; i < intStep; i++)
                {
                    strTemp = String.Format(
                        "<img src=\"{0}\" height=\"{1}\" width=\"{2}\">", "/images/blank.gif", "0", (intStep * 15));
                }
        ...
        #region ConvertToFileSize() Function
        ///<summary>
        ...
        /// <returns></returns>
        public static string FuncFileDwonSingle(int id, string strFileName, string strFileSize)
        {
            if (strFileName.Length > 0)
            {
                return "<a href=\"/CoreMvcAppDemo/BoardDown?Id="
                    + id.ToString() + "\">"
        ...
        /// <returns></returns>
        private static string DownloadType(string strFileName, string altString)
        {
